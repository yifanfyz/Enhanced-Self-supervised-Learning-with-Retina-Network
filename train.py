...
LABELED_DATASET_PATH = "data/labeled_data"
UNLABELED_DATASET_PATH = "data/unlabeled_data"

## train settings
save_dir = "~/saved_weight"
pretrained = None
labeled_epochs = 128
...
def train_labeled(model, data_loader, device):
    coco_loader = convert_to_coco_api(data_loader)
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-5)
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, patience=3, verbose=True)
    
    for epoch_num in range(labeled_epochs):
        model.train()
        epoch_loss = []
        for iter_num, data in enumerate(coco_loader):
            try:
                optimizer.zero_grad()
                img, target = data
                if torch.cuda.is_available():
                    classification_loss, regression_loss = model(img)
...
def train_unlabeled(model, data_loader, device):
    ##TODO
    pass
...
def main():
    ...
    unlabeled_dataset = LabeledDataset(
    ...
    train_labeled(model, labeled_loader, device=device)
    train_unlabeled(model, unlabeled_dataloader, device=device)
